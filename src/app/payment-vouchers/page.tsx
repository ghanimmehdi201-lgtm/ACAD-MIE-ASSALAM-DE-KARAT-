...
import { printDocument, generatePaymentVoucherHTML } from "@/lib/print";
import { exportPaymentVouchersToExcel } from "@/lib/export";
import { Plus, Search, Printer, Trash2, Edit, Copy, Download } from "lucide-react";
...
export default function PaymentVouchersPage() {
  ...
  const handleSubmit = async (data: VoucherFormData) => {
    ...
    try {
      const body = {
        ...
        date: data.date,
        paidTo: data.personName,
        paidToAr: data.personNameAr,
        ...
        purposeAr: data.descriptionAr,
        paymentMethod: data.paymentMethod,
        chequeNumber: data.chequeNumber,
        notes: data.notes,
      };

      const url = editVoucher ? `/api/payment-vouchers/${editVoucher.id}` : "/api/payment-vouchers";
  ...
  const handleDelete = async () => {
    if (!deleteId) return;
    await fetch(`/api/payment-vouchers/${deleteId}`, { method: "DELETE" });
    toast.success(t(lang, "deleteSuccess"));
    loadVouchers();
  };

  const handleDuplicate = (voucher: PaymentVoucher) => {
    setEditVoucher({
      ...voucher,
      id: 0,
      voucherNumber: generateVoucherNumber(
        settings?.paymentVoucherPrefix || "BC",
        activeSeason?.paymentVoucherCounter || 0,
  ...
  const handlePrint = (voucher: PaymentVoucher) => {
    if (!settings) return;
    const html = generatePaymentVoucherHTML(voucher, settings, (settings.documentLanguage as "fr" | "ar" | "bilingual") || "bilingual");
    printDocument(html);
  };

  const getPaymentBadge = (method: string) => {
    const variants: Record<string, "info" | "success" | "warning" | "default"> = {
      cash: "success", cheque: "info", transfer: "warning",
    };
    const labels: Record<string, string> = { cash: "Espèces", cheque: "Chèque", transfer: "Virement" };
    return <Badge variant={variants[method] || "default"}>{labels[method] || method}</Badge>;
  };

  if (!activeSeason) {
    return (
      <AppLayout title={t(lang, "paymentVouchers")}>
  ...
  return (
    <AppLayout title={t(lang, "paymentVouchers")}>
      <div className="space-y-4">
        <div className="flex flex-wrap items-center gap-3">
          ...
          <Button
            variant="outline"
            size="sm"
            onClick={() => exportPaymentVouchersToExcel(vouchers, settings!, activeSeason.name)}
          >
            <Download className="h-4 w-4" />
            Excel
          </Button>
          <Button onClick={() => { setEditVoucher(null); setShowForm(true); }}>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-slate-50 border-b border-slate-200">
                  ...
                  <th className="px-4 py-3 text-left font-semibold text-slate-600">{t(lang, "paidTo")}</th>
                  <th className="px-4 py-3 text-left font-semibold text-slate-600">{t(lang, "purpose")}</th>
                ...
                {loading ? (
                  ...
                  vouchers.map((v) => (
                    <tr key={v.id} className="border-b border-slate-100 hover:bg-slate-50 transition-colors">
                      <td className="px-4 py-3 font-mono text-orange-700 font-medium">{v.voucherNumber}</td>
                      ...
                      <td className="px-4 py-3 text-slate-600 max-w-32 truncate">{v.purpose}</td>
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center justify-center gap-1">
                          <button onClick={() => handlePrint(v)} className="p-1.5 rounded hover:bg-slate-100 text-slate-500 hover:text-slate-700" title={t(lang, "print")}>
                            <Printer className="h-4 w-4" />
                          </button>
                          <button onClick={() => { setEditVoucher(v); setShowForm(true); }} className="p-1.5 rounded hover:bg-blue-50 text-slate-500 hover:text-blue-600" title={t(lang, "edit")}>
                            <Edit className="h-4 w-4" />
                          </button>
                          <button onClick={() => handleDuplicate(v)} className="p-1.5 rounded hover:bg-slate-100 text-slate-500 hover:text-slate-700" title={t(lang, "duplicate")}>
                            <Copy className="h-4 w-4" />
                          </button>
                          <button onClick={() => setDeleteId(v.id)} className="p-1.5 rounded hover:bg-red-50 text-slate-500 hover:text-red-600" title={t(lang, "delete")}>
              ...
              {vouchers.length > 0 && (
                <tfoot>
                  <tr className="bg-red-50 border-t-2 border-red-200">
                    <td colSpan={5} className="px-4 py-3 font-semibold text-slate-700">Total ({vouchers.length} bons)</td>
                    <td className="px-4 py-3 text-right font-bold text-red-700 text-base">
                      {formatCurrency(vouchers.reduce((s, v) => s + parseFloat(v.amount ?? "0"), 0), settings?.currency || "MAD")}
                    </td>
                    <td />
                  </tr>
                </tfoot>
      ...
      <Modal
        ...
        title={editVoucher?.id ? t(lang, "edit") + " BC" : t(lang, "new") + " Bon de Caisse"}
        size="xl"
      >
        <VoucherForm
          type="payment"
          initialData={editVoucher ? {
            ...
            personName: editVoucher.paidTo,
            personNameAr: editVoucher.paidToAr || "",
            ...
            description: editVoucher.purpose || "",
            descriptionAr: editVoucher.purposeAr || "",
          ...
          loading={saving}
          voucherPrefix={settings?.paymentVoucherPrefix || "BC"}
          counter={activeSeason?.paymentVoucherCounter || 0}
