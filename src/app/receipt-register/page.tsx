...
<head><meta charset="UTF-8"><title>Registre Recettes</title>
<style>
...
<div class="header">
  <h1>${settings?.name}</h1>
  <h2>${t(lang, "receiptVoucherRegisterTitle")} – ${activeSeason?.name}</h2>
</div>
<table>
  <thead>
    <tr>
  ...
  if (!activeSeason) {
    return (
      <AppLayout title={t(lang, "receiptRegister")}>
    ...
    <AppLayout title={t(lang, "receiptRegister")}>
      <div className="space-y-4">
        <div className="flex flex-wrap items-center gap-3">
          ...
          <Button variant="outline" size="sm" onClick={() => exportReceiptVouchersToExcel(vouchers, settings!, activeSeason.name)}>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-slate-50 border-b border-slate-200">
                  ...
                  <th className="px-3 py-3 text-left font-semibold text-slate-600">{getRegisterLabel(regLang, "payer")}</th>
                  ...
                  <th className="px-3 py-3 text-left font-semibold text-slate-600">{getRegisterLabel(regLang, "reason")}</th>
                  <th className="px-3 py-3 text-left font-semibold text-slate-600">{getRegisterLabel(regLang, "paymentMethod")}</th>
                  <th className="px-3 py-3 text-right font-semibold text-emerald-600">{getRegisterLabel(regLang, "amount")}</th>
                </tr>
              </thead>
              <tbody>
                {loading ? (
                  ...
                  vouchers.map((v, i) => (
                    <tr key={v.id} className="border-b border-slate-100 hover:bg-slate-50">
                      ...
                      <td className="px-3 py-2.5 font-mono text-emerald-700 text-xs">{v.voucherNumber}</td>
                      <td className="px-3 py-2.5 text-slate-600">{formatDate(v.date)}</td>
                      <td className="px-3 py-2.5 font-medium text-slate-800">{v.receivedFrom}</td>
                      ...
                      <td className="px-3 py-2.5 text-slate-600 max-w-32 truncate">{v.reason}</td>
